...
| Prod | `payment-gateway-prod-root` | `payment-gateway-prod-fraud-service` | `fraud-service` | `values-prod.yaml` pins `v1.1.0` |
...
Dev follows the moving GHCR `latest` image and always pulls it. Production continues to track `main` but pins its image tag in `values-prod.yaml` until a production image promotion policy is established. The release workflow also publishes immutable commit-SHA tags.
